...
export default class CodeActionProvider implements VSCodeCodeActionProvider {
  ...
  private static readonly ReadonlyRegex = new RegExp(
    /(public|private|protected)\s(\w+)\s(\w+)\s?{\s?(get;)\s?(private\s)?(set;)?\s?}/g
  );
  private static readonly ClassRegex = new RegExp(
    /(private|internal|public|protected)\s?(static)?\sclass\s(\w*)/g
  );
  private static readonly GeneralRegex = new RegExp(
    /(public|private|protected)\s(.*?)\(([\s\S]*?)\)/gi
  );

  constructor() {
    commands.registerCommand(
      this._commandIds.initializeMemberFromCtor,
      this.initializeMemberFromCtor,
      this
    );
    commands.registerCommand(
      this._commandIds.ctorFromProperties,
      this.executeCtorFromProperties,
      this
    );
  }
  ...
  public provideCodeActions(
    ...
    const addInitalizeFromCtor = (type: MemberGenerationType) => {
      const action = this.getInitializeFromCtorAction(
        document,
        range,
        context,
        token,
  ...
  private async executeCtorFromProperties(
    args: ConstructorFromPropertiesArgument
  ) {
    ...
    const assignments = args.properties.map(
      (prop) =>
        `${Array(tabSize * 1).join(' ')} this.${prop.name} = ${this.camelize(
          prop.name
        )};${os.EOL}`
    );

    const firstPropertyLine = args.properties.sort(
      (a, b) => a.lineNumber - b.lineNumber
    )[0].lineNumber;

    const ctorStatement = `${Array(tabSize * 2).join(' ')} ${
      args.classDefinition.modifier
    } ${args.classDefinition.className}(${ctorParams.join(', ')}) 
        {
        ${assignments.join('')}   
        }
        `;
  ...
  private async formatDocument(
    ...
    const reFormatAfterChange = workspace
      .getConfiguration()
    ...
    if (reFormatAfterChange) {
      try {
        const formattingEdits = await commands.executeCommand<TextEdit[]>(
  ...
  private findClassFromLine(
    document: TextDocument,
    lineNo: number
  ): CSharpClassDefinition | null {
  ...
  private async initializeMemberFromCtor(args: InitializeFieldFromConstructor) {
    ...
    const bodyStartRange = new Range(
      args.constructorBodyStart,
      args.constructorBodyStart
    );
    const declarationRange = new Range(
      args.constructorStart,
      args.constructorStart
    );

    const declarationEdit = new TextEdit(
      declarationRange,
      args.memberGeneration.declaration
    );
    const memberInitEdit = new TextEdit(
      bodyStartRange,
      args.memberGeneration.assignment
    );
    ...
    if (
      args.document
        .getText()
        .indexOf(args.memberGeneration.declaration.trim()) === -1
    )
    ...
    if (
      args.document
        .getText()
        .indexOf(args.memberGeneration.assignment.trim()) === -1
    )
  ...
  private getInitializeFromCtorAction(
    document: TextDocument,
    range: Range,
    context: CodeActionContext,
    token: CancellationToken,
    ...
    const position = editor.selection.active;
    const positionStart = new Position(
      position.line < 2 ? 0 : position.line - 2,
      0
    ); // Limit line to start of file
    const positionEnd = new Position(
      document.lineCount - position.line < 2 ? 0 : position.line + 2,
      0
    ); // Limit line to end of file
    ...
    const lineText = editor.document.getText(
      new Range(position.line, 0, position.line, wordRange.end.character)
    );
    const selectedName = lineText.substr(
      wordRange.start.character,
      wordRange.end.character - wordRange.start.character
    );
    ...
    switch (memberGenerationType) {
      case MemberGenerationType.PrivateField:
        ...
        memberGeneration = {
          ...
          declaration: `${Array(tabSize * 2).join(
            ' '
          )} private readonly ${parameterType} ${privateMemberPrefix}${selectedName};\r\n`,
          assignment: `${Array(tabSize * 3).join(' ')} ${
            prefixWithThis ? 'this.' : ''
          }${privateMemberPrefix}${selectedName} = ${selectedName};\r\n`,
        };
        break;
      ...
      case MemberGenerationType.Property:
        ...
        memberGeneration = {
          ...
          declaration: `${Array(tabSize * 2).join(
            ' '
          )} public ${parameterType} ${name} { get; set; }\r\n`,
          assignment: `${Array(tabSize * 3).join(' ')} ${
            prefixWithThis ? 'this.' : ''
          }${name} = ${selectedName};\r\n`,
        };
    ...
    const constructorBodyStart = this.findConstructorBodyStart(
      document,
      position
    );
  ...
  private findConstructorBodyStart(
    document: TextDocument,
    position: Position
  ): Position | null {
    for (let lineNo = position.line; lineNo < position.line + 5; lineNo++) {
      ...
      if (line.text.indexOf('{') !== -1) return new Position(lineNo + 1, 0);
    }

    return null;
  }

  private findConstructorStart(
    document: TextDocument,
    position: Position
  ): Position {
    ...
    if (foundClass) {
      for (let lineNo = position.line; lineNo > position.line - 5; lineNo--) {
        ...
        if (
          ...
          !(line.lineNumber < foundClass.startLine)
        )
